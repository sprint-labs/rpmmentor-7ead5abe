...
- The live RPM database is Supabase project `zdxxezquhvpjmoxlecjp`. Project MCP config is `.cursor/mcp.json` (Desktop). Cloud Agents do **not** load that file automatically — add the same HTTP server in the cursor.com/agents MCP settings and complete OAuth. Do not use Lovable Cloud `query_database` as a substitute. Treat the Supabase MCP as read-only unless the owner explicitly asks for a production data or schema change.
