...
export interface Goalkeeper {
  ...
  name: string;
  initials: string;
  /** Null for a goalkeeper management has not tiered yet. */
  status: Status | null;
  /** Null for a goalkeeper management has not tiered yet. */
  tier: TierLevelLabel | null;
  /** Academy / Free Agent. Independent of the tier — both can apply at once. */
  tags: GoalkeeperTag[];
  /** Null for a goalkeeper management has not tiered yet. */
...
function deriveRegion(s: Seed): Region {
  return regionForLeague(s.league);
}

/**
 * Where a goalkeeper plays, from the league alone.
 *
 * A pure function of one live column, so the live roster derives it exactly as
 * the seed did rather than storing a second copy of the same fact.
 */
export function regionForLeague(league: string): Region {
  if (league === "Free Agent") return "Free Agent";
  return OVERSEAS_LEAGUES.has(league) ? "Overseas" : "UK Based";
}
function deriveTierLevel(s: Seed): TierLevel {
  return s.tier;
}
/**
 * "June 2028" to "2028-06-30".
 *
 * `players.contract_until` stores the month-and-year form, while every sort,
 * expiry filter and year dropdown in the app assumes ISO. Exported so the live
 * roster converts exactly as the seed pipeline does.
 */
export function contractISO(c: string): string {
  if (c === "—") return "—";
...
export function dutyStatusForGk(
  ...
  if (!gk.status) return dutyResult("not_enough_data", 0);
