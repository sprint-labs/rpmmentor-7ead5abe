      ...
      <Card className="p-3">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            Filters{" "}
            {activeFilterCount > 0 && (
              <span className="px-1.5 rounded bg-primary/15 text-primary text-[10px]">
                {activeFilterCount}
              </span>
            )}
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-2">
          ...
          <select
            ...
            onChange={(e) =>
              setFilters((f) => ({ ...f, kind: e.target.value as MediaKind | "all" }))
            }
            className="h-9 px-2 rounded-md bg-input/60 border border-border text-sm"
          >
            {KINDS.map((k) => (
          ...
          <select
            value={filters.gkId ?? ""}
            onChange={(e) => setFilters((f) => ({ ...f, gkId: e.target.value || undefined }))}
            className="h-9 px-2 rounded-md bg-input/60 border border-border text-sm"
          >
          ...
          <select
            ...
            onChange={(e) =>
              setFilters((f) => ({ ...f, uploaderName: e.target.value || undefined }))
            }
            className="h-9 px-2 rounded-md bg-input/60 border border-border text-sm"
          >
            <option value="">All uploader names</option>
            {Array.from(
              new Set([...DEMO_USERS.map((u) => u.name), ...uploaders.map((u) => u.name)]),
            ).map((name) => (
          ...
          <div className="flex gap-1">
            <input
              ...
              className="w-full h-9 px-2 rounded-md bg-input/60 border border-border text-xs"
            />
            <input
              type="date"
      ...
      {!loading && assets.length === 0 ? (
        <Card>
          <EmptyState
            ...
            title={
              activeFilterCount > 0 ? "No media matches these filters" : "No media uploaded yet"
            }
            description={
              activeFilterCount > 0
              ...
              <Card key={m.id} className="overflow-hidden group">
                ...
                <div className="p-3">
                  ...
                  <div className="text-[11px] text-muted-foreground mt-1">
                    {gk?.name ?? "Unknown goalkeeper"}
                  </div>
                  {m.rating_tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {m.rating_tags.slice(0, 3).map((t) => (
                        <Pill key={t} tone="info">
                          {t}
                        </Pill>
                      ))}
                      {m.rating_tags.length > 3 && (
