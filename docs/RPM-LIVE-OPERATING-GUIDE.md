...
- **Sign-in providers: confirm the live provider state before the release, not after it.** Any release that adds, enables or changes a sign-in route must be evidenced against the live Supabase project *before* merging, because every precondition sits outside this repository and a green build proves none of them. For Google, open [Authentication → Sign In / Providers](https://supabase.com/dashboard/project/zdxxezquhvpjmoxlecjp/auth/providers?provider=Google) and confirm: the provider toggle is **on**; the saved client ID is a real Google `*.apps.googleusercontent.com` value and not a placeholder; that Google client's Authorised redirect URIs include `https://zdxxezquhvpjmoxlecjp.supabase.co/auth/v1/callback`; and [URL Configuration](https://supabase.com/dashboard/project/zdxxezquhvpjmoxlecjp/auth/url-configuration) lists `https://www.rpmmentor.com/**`. Then complete one real sign-in on the preview deployment. If any of those cannot be evidenced, ship with `GOOGLE_SIGN_IN_ENABLED` set to `false` in `src/lib/auth-providers.ts` and enable it in a follow-up once they can.
...
3. ~~Reconcile the repository's migration history with the live database into a reviewed forward-only baseline before the next schema project.~~ — **Done 20 Sep 2026.** The baseline is `docs/supabase-production-migration-manifest.json` (52 repository files, 52 live ledger rows) and `npm run check:migrations` re-verifies it in `pull-request-validation.yml`. A drift now fails the pull request rather than waiting to be noticed. Recapture the manifest whenever a forward migration is applied live.
